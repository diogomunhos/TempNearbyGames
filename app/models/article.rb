class Article < ActiveRecord::Base
	...
	scope :getLast10Articles, -> (slider = nil) { where.not(id: slider).order('created_at desc').limit(10).includes(:documents) }	
	scope :getLast4ArticlesSlider, -> { where(is_highlight: true).order('created_at desc').limit(4).includes(:documents) }
